#!/usr/bin/env python3
"""
Reorganize challenges by extracting line ranges
"""
...
# Find line numbers where each challenge starts (looking for "  {")
challenge_starts = []
for i, line in enumerate(lines):
    if line.strip().startswith('{'):
        challenge_starts.append(i)

print(f"Found {len(challenge_starts)} challenge start lines")

# For each challenge, find its end by counting braces
challenges = []
for start_idx in challenge_starts:
    challenge_lines = [lines[start_idx]]
    brace_count = 1
    i = start_idx + 1
    while i < len(lines) and brace_count > 0:
        ...
        i += 1
    challenges.append(''.join(challenge_lines))

print(f"Extracted {len(challenges)} challenges")

# Now proceed with reorganization as before
import re
...
# Write file
header = ''.join(lines[:14])  # Interface definition and array start
new_content = header
